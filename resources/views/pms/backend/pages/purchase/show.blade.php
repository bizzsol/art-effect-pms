                    ...
                    <div class="invoice-details mt25 row">
                        ...
                        <div class="well col-6">
                            <ul class="list-unstyled mb0">
                                <li>
                                    <div class="ratings">
                                        <a href="{{route('pms.supplier.profile',$purchaseOrder->relQuotation->relSuppliers->id)}}"
                                           target="_blank"><span>Rating:</span></a>
                                ...
                                <li><strong>Supplier
                                        :</strong> {{isset($purchaseOrder->relQuotation->relSuppliers->name)?$purchaseOrder->relQuotation->relSuppliers->name:''}}
                                </li>
                                <li><strong>Code
                                        :</strong> {{isset($purchaseOrder->relQuotation->relSuppliers->code)?$purchaseOrder->relQuotation->relSuppliers->code:''}}
                                </li>
                                <li><strong>Email
                                        :</strong> {{isset($purchaseOrder->relQuotation->relSuppliers->email)?$purchaseOrder->relQuotation->relSuppliers->email:''}}
                                </li>
                                <li><strong>Phone
                                        :</strong> {{isset($purchaseOrder->relQuotation->relSuppliers->phone)?$purchaseOrder->relQuotation->relSuppliers->phone:''}}
                                </li>
                                <li><strong>Mobile
                                        :</strong> {{isset($purchaseOrder->relQuotation->relSuppliers->mobile_no)?$purchaseOrder->relQuotation->relSuppliers->mobile_no:''}}
                                </li>
                            </ul>
                        </div>
                        <div class="col-6">
                            <ul class="list-unstyled mb0 pull-right">
                                ...
                                @if(isset($purchaseOrder->relQuotation->delivery_date))
                                    <li><strong>Delivery Date
                                            :</strong> {{date('d-m-Y',strtotime($purchaseOrder->relQuotation->delivery_date))}}
                                    </li>
                                @endif
                                <li><strong>Reference No:</strong> {{$purchaseOrder->reference_no}}</li>
                                <li><strong>Quotation
                                        No:</strong> {{isset($purchaseOrder->relQuotation->reference_no)?$purchaseOrder->relQuotation->reference_no:''}}
                                </li>
                            </ul>
                        </div>
                    </div>
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tr class="text-center">
                            ...
                            <th>Item Total ({{ $currency }})</th>
                            @if(!$same)
                                <th>Item Total ({{ $systemCurrency->code }})</th>
                            @endif
                        </tr>
                        </thead>
                        <tbody>
                        ...
                        @foreach($purchaseOrder->relPurchaseOrderItems as $key=>$item)
                            <tr>
                                ...
                                <td>{{isset($item->relProduct->name)?$item->relProduct->name:''}}
                                    ...
                                    {{ getProductAttributesFaster($requisitionItems->where('uid', $item->uid)->first()) }}</td>
                        ...
                        <tr>
                            ...
                            <td class="text-right">{{systemMoneyFormat($purchaseOrder->relPurchaseOrderItems->sum('unit_price'))}}</td>
                        ...
                        <tr>
                            ...
                            <td class="text-right"><strong>{{systemMoneyFormat($purchaseOrder->gross_price)}}</strong>
                            </td>
                            @if(!$same)
                                <td class="text-right">
                                    <strong>{{systemMoneyFormat($purchaseOrder->gross_price*$exchangeRate)}}</strong>
                                </td>
                            @endif
                        </tr>
                        </tbody>
