            ...
            <div class="page-content">
                <div class="">
                    <div class="panel panel-info">
                        <form method="post" action="{{ route('pms.purchase.revise.po.update',$purchaseOrder->id) }}"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-2 col-sm-12">
                                        <p class="mb-1 font-weight-bold"><label
                                                    for="rfp_reference_no"><strong>{{ __('CS Number') }} <span
                                    ...
                                    <div class="col-md-2 col-sm-12">
                                        ...
                                        <div class="input-group input-group-md mb-3 d-">
                                            <input type="date" name="delivery_date" id="delivery_date"
                                                   class="form-control rounded" aria-label="Large" min
                                                   aria-describedby="inputGroup-sizing-sm" required
                                            ...
                                            <select name="hr_unit_id" id="hr_unit_id" class="form-control rounded"
                                                ...
                                                @if($units->count() > 0)
                                                    ...
                                                    @foreach($units as $key => $unit)
                                                        <option value="{{ $unit->hr_unit_id }}" {{$unit->hr_unit_id==$purchaseOrder->hr_unit_id?'selected':''}}>{{ $unit->hr_unit_name }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                            ...
                                            <select name="warehouse_id"
                                                ...
                                                @foreach($warehouses as $warehouse)
                                                    <option value="{{ $warehouse->id }}"
                                                            {{$warehouse->id==$purchaseOrder->warehouse_id?'selected':''}}}>
                                                        {{ $warehouse->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-md-{{$uncommon>0?2:4}} col-sm-12">
                                        <p class="mb-1 font-weight-bold"><label
                                ...
                                <div class="form-row">
                                    <div class="col-md-12">
                                        ...
                                        <table class="table table-bordered">
                                            ...
                                            <tbody id="milestones">
                                            @if(!empty($purchaseOrder->milestones) && count($purchaseOrder->milestones) > 0)
                                                @foreach($purchaseOrder->milestones as $milestone)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <input type="number"
                                                                   name="milestone_percentages[]"
                                                                   min="0" max="100"
                                                                   value="{{ $milestone->percentage ?? 0 }}"
                                                                   class="form-control text-right milestones"
                                                                   onchange="calculateMilestone($(this))"
                                                                   onkeyup="calculateMilestone($(this))"
                                                                   required>
                                                        </td>
                                                        <td class="text-center">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="input-group input-group-md mb-3 d-">
                                            <textarea name="remarks"
                                                      ...
                                                      rows="2"
                                                      cols="5"
                                                      placeholder="Write here...">{{$purchaseOrder->remarks}}</textarea>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        ...
                                        <div class="input-group input-group-md mb-3 d-">
                                            <textarea name="terms"
                                                      ...
                                                      class="form-control rounded summer-editor"
                                                      rows="3"
                                                      placeholder="Write here...">{{$purchaseOrder->terms}}</textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-12 text-right mt-10">
                                        <button type="submit" class="btn btn-success rounded"><i
                                                    class="las la-redo-alt"></i> {{ __('Revised PO') }}</button>
                                    </div>
                                </div>
...
@section('page-script')
    <script>
        ...
        function getRequisitions() {
            ...
            let quotationId = $('.quotation-id').val();
            $.ajax({
                url: "{{url('pms/purchase/unit-wise-requisition')}}/" + hrUnitId + '/' + quotationId + "?hr_department_id=" + hr_department_id + "&uncommon={{ $uncommon }}",
                type: 'GET',
                dataType: 'json',
                data: {},
            })
                .done(function (response) {
                    var selected_requisitions = <?php echo json_encode($purchaseOrder->purchaseOrderRequisitions->pluck('requisition_id')->toArray()); ?>;
                    var requisition = '<option value="">Select Requisition</option>';
                    $.each(response, function (index, val) {
                        requisition += '<option value="' + val.id + '" ' + ($.inArray(val.id, selected_requisitions) !== -1 ? 'selected' : '') + '>' + val.reference_no + '</option>';
                    });
