                            ...
                            <div class="col-md-3">
                                ...
                                <div class="select-search-group input-group input-group-md mb-3 d-">
                                    <select name="category_id" id="category_id" class="form-control rounded select2" required>
                                        ...
                                        {!! categoryOptions([]) !!}
                                        {!! categoryOptions([], 0, 0, 1, 0) !!}
                                        {!! categoryOptions([], 0, 0, 0, 1) !!}
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-9">
                                <p class="mb-1 font-weight-bold"><label for="name"><strong>{{ __('Quality Issues') }}<span class="text-danger">&nbsp;*</span></strong></label> {!! $errors->has('name')? '<span class="text-danger text-capitalize">'. $errors->first('name').'</span>':'' !!}</p>
                                <div class="input-group input-group-md mb-3 d-">
                                    <input type="text" name="name" id="name" class="form-control rounded" aria-label="Large" placeholder="{{__('Write Quality Issue')}}" aria-describedby="inputGroup-sizing-sm" required value="{{ old('name') }}">
                                </div>
                            </div>
                            <div class="col-md-2">
