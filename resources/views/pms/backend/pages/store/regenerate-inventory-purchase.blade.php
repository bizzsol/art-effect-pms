            ...
            <div class="page-content">
                <div class="">
                    <div class="panel panel-info">
                        <form method="post" id="updateInventoryForm" action="{{ route('pms.rfp.regenerate-po.purchase') }}" enctype="multipart/form-data">
                        @csrf
                            <div class="panel-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="row">
                                            <input type="hidden" name="old_quotation" value="{{$QuotationDetails->id}}">
                                            <input type="hidden" name="old_requisition" value="{{$firstRequisition->requisition_id}}">
                                            <input type="hidden" name="old_requestProposal" value="{{$firstRequisition->request_proposal_id}}">
                                            <input type="hidden" name="reference_no" value="">
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <div class="form-line">
                                                        {!! Form::label('request_date', 'Quotation Date', array('class' => 'mb-1 font-weight-bold')) !!}
                                            ...
                                            <div class="col-md-4 col-sm-12" id="assign-user-section">
                                            ...
                                            <div class="col-md-4 col-sm-12 mt-4">
                                                <a href="javascript:void(0)" onclick="openFullViewModal({{$requestProposal->id}})"  class="btn btn-info">
                                                    <i class="las la-eye"></i> See Old CS
                                                </a>
                                            </div>
                                    ...
                                    <table class="table table-striped table-bordered table-head direct-purchase-table" cellspacing="0" width="100%" id="dataTable">
                                        ...
                                        @if(isset($proDetails))
                                            @php
                                                ...
                                                $sumOfVat=0;
                                            @endphp
                                            @foreach($proDetails as $key=>$item)
                                                <tr>
                                                    <td>
                                                        {{ isset($item->relProduct->name) ? $item->relProduct->name : '' }} ({{ isset($item->relProduct->sku) ? $item->relProduct->sku : '' }}) {{ getProductAttributesFaster($item->relProduct) }} {{ getProductAttributesFaster($item) }}
                                                        <input type="hidden" name="product_id[]" class="form-control" value="{{$item->uid}}">
                                                    </td>
                                                    <td>
                                                        <textarea name="product_description[{{$item->uid}}]" class="form-control" style="min-height: 80px;">{{ getProductAttributesFaster($item) }}</textarea>
                                                    </td>
                                                    <td class="text-center">
                                                        {{$item->relProduct->productUnit->unit_name}}
                                                    </td>
                                                    <td>
                                                        <input type="hidden" name="request_qty[{{$item->uid}}]" value="{{round($item->approved_qty)}}">
                                                        <input type="number" name="qty[{{$item->uid}}]" required
                                                               class="form-control summation-qty" min="0"
                                                               id="qty{{$item->uid}}"
                                                               value="{{round($item->approved_qty)}}"
                                                    ...
                                                    <td>
                                                        <input type="number"
                                                               ...
                                                               placeholder="0.00" value="{{ $item->unit_price*$item->approved_qty }}">
                                        ...
                                        <tr class="total-row">
                                            ...
                                            <td colspan="2" class="text-right total_value_after_discount">
                                        ...
                                        <tr class="total-row">
                                            ...
                                            <td colspan="2" class="text-right" id="total_vat_amount">
                                        ...
                                        <tr class="total-row">
                                            ...
                                            <td colspan="2" class="text-right" id="total_gross_amount">
...
@section('page-script')
    <script>
        ...
        function getSupplierInfo() {
            ...
            $.ajax({
                url: "{{ url('pms/rfp/send-to-purchase/'.$quotation_id) }}?get-supplier-info&supplier_id=" + $('#supplier_id').val(),
                type: 'GET',
                dataType: 'json',
                data: {},
            })
