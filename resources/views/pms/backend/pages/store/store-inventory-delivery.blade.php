                ...
                <div class="">
                    <div class="panel panel-info">
                        <form method="post" id="updateInventoryForm"
                              action="{{ route('pms.store-manage.store-requisition.submit') }}">
                            @csrf
                            <div class="panel-body">
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <div class="form-line">
                                                ...
                                                <input type="text" id="requisition_reference_no" name=""
                                            ...
                                            <div class="form-line">
                                                ...
                                                @if ($errors->has('reference_no'))
                                                    <span class="help-block"><strong
                                                                class="text-danger">{{ $errors->first('reference_no') }}</strong>
            </span>
                                                @endif
                                            </div>
                                        </div>
                                                ...
                                                @if(isset($item->product->id))
                                                    @if($item->qty != $item->delivery_qty)
                                                        ...
                                                        <tr id="SelectedRow{{$item->uid}}">
                                                            ...
                                                            <td class="text-center">
                                                                <input type="hidden"
                                                                       value="{{$item->qc_qty-$item->delivery_qty}}"
                                                                       id="leftQty_{{$item->uid}}">
                                                                <span id="leftQtyAfterSubTract_{{$item->uid}}">{{$item->qc_qty-$item->delivery_qty}}</span>
                                                            </td>
                                                            <td class="text-center">
                                                                <select name="product_unit_id[{{ $item->uid }}]"
                                                                        id="product_unit_id_{{ $item->uid }}"
                                                                        onchange="updateDeliveryQuantity($(this))"
                                                                        class="product_unit_id">
                                                                    <option value="{{ $item->product->product_unit_id }}"
                                                                            data-conversion-rate="1">{{ $item->product->productUnit->unit_name }}</option>
                                                                    @if($item->product->productUnitConversions->where('conversion_rate', '>', 0)->count() > 0)
                                                                        @foreach($item->product->productUnitConversions->where('conversion_rate', '>', 0) as $key => $this_unit)
                                                                            <option value="{{ $this_unit->conversion_unit_id }}"
                                                                                    data-conversion-rate="{{ $this_unit->conversion_rate }}">{{ $this_unit->conversionUnit->unit_name }}
                                                                                ({{ $this_unit->conversion_rate }})
                                                                            </option>
                                                                        @endforeach
                                                                    @endif
                                                                </select>
                                                            ...
                                                            <td>
                                                                <input type="number" name="delivery_qty[{{$item->uid}}]"
                                                                       id="delivery_qty_{{$item->uid}}"
                                                                       class="form-control delivery_qty text-right"
                                                                       min="0" max="{{$left}}" value="{{$left}}"
                                                                       data-id="{{$item->uid}}" step="1"
                                                                       onchange="checkDeliveryQuantity($(this))"
                                                                       onkeyup="checkDeliveryQuantity($(this))">
                                                            </td>
                                                            <td>
                                                                <select class="form-control not-select2 warehouse_id"
                                                                        name="warehouse_id[{{$item->uid}}]"
                                                                        id="warehouse_{{$item->uid}}"
                                                                        onchange="updateDeliveryQuantity($(this))">
                                                                    @if($warehouses->whereIn('id', array_keys($stocks[$item->id]))->count() > 0)
                                                                        @foreach($warehouses->whereIn('id', array_keys($stocks[$item->id])) as $warehouse)
                                                                            <option value="{{$warehouse->id}}"
                                                                                    data-remaining="{{ $stocks[$item->id][$warehouse->id] }}"> {!! $warehouse->name. ' ('.$stocks[$item->id][$warehouse->id].' '.$item->product->productUnit->unit_name.')' !!}</option>
                                                                        @endforeach
                                                                    @endif
                                                                </select>
                                                            </td>
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="col-12 text-right">
                                        <button type="submit"
                                                class="btn btn-success rounded">{{ __('Confirm Delivery') }}</button>
                                    </div>
                                </div>
                            </div>
                        </form>
