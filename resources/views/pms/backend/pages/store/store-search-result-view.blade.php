<table class="table table-striped table-bordered table-head datatable-exportable" id="dataTable"
       data-table-name="Store Search" border="1">
    <thead>
                        ...
                        <ul class="dropdown-menu">
                            ...
                            @can('send-to-rfp')
                                @if($values->is_send_to_rfp=='no')
                                    <li>
                                        <a class="sendToPurchaseDepartment"
                                           data-src="{{route('pms.store-manage.send.to.purchase.department')}}"
                                           data-id="{{$values->id}}"
                                           title="Send To Procurement">{{ __('Send To Procurement')}}
                                        </a>
                                    </li>
                                @endif
                            @endcan
