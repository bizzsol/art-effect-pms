            ...
            <div class="page-content">
                <div class="">
                    <div class="panel panel-info">
                        ...
                        <form action="{{ route('pms.requisition.requisition.update',$requisition->id) }}?redirect={{ request()->get('redirect') }}&editor={{ request()->get('editor') }}"
                              method="POST" id="editRequisitionForm" enctype="multipart/form-data"
                            ...
                            <div class="panel-body">
                                <div class="row">
                                    <div class="col-md-2 col-sm-6">
                                        <p class="mb-1 font-weight-bold"><label
                                                    for="reference">{{ __('Reference No.') }}:</label></p>
                                        <div class="input-group input-group-md mb-3 d-">
                                            <input type="text" name="reference_no" id="reference"
                                                   ...
                                                   value="{{ old('reference_no',$requisition->reference_no) }}">
                                    ...
                                    <div class="col-md-2 col-sm-6">
                                        ...
                                        <div class="input-group input-group-md mb-3 d-">
                                            <input type="datetime-local" name="requisition_date" id="date"
                                                   class="form-control rounded" aria-label="Large"
                                                   aria-describedby="inputGroup-sizing-sm" required
                                                   value="{{ date('Y-m-d H:i:s', strtotime($requisition->requisition_date)) }}">
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-2 col-sm-6">
                                        ...
                                        <div class="input-group input-group-md mb-3 d-">
                                            <select name="saleable" id="saleable" class="form-control">
                                                <option value="yes" {{ $requisition->saleable == 'yes' ? 'selected' : '' }}>
                                                    Yes
                                                </option>
                                                <option value="no" {{ $requisition->saleable == 'no' ? 'selected' : '' }}>
                                                    No
                                                </option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="input-group input-group-md mb-3 d-">
                                            @if($requisition->status==1)
                                                <input type="text" name="category_id"
                                                       value="{{isset($requisition->items[0]->product->category->category->name)?$requisition->items[0]->product->category->category->name:''}}"
                                                       class="form-control" readonly>
                                            @else

                                                <select name="category_id" id="category_id"
                                                        class="form-control category" onchange="getSubCategories()">
                                                    @if(isset($categories[0]))
                                                        <optgroup label="Products">
                                                            @foreach($categories as $category)
                                                                <option value="{{ $category->id }}"
                                                                        {{ $category_id == $category->id ? 'selected' : '' }} data-sub-category-ids="{{ $category->subCategory->pluck('id')->implode(',') }}"
                                                                        data-sub-category-names="{{ $category->subCategory->pluck('name')->implode(',') }}"
                                                                        data-sub-category-codes="{{ $category->subCategory->pluck('code')->implode(',') }}">{{
                                    ...
                                    @if(isset($task->id))
                                        <div class="col-md-3 col-sm-6">
                                            ...
                                            <div class="input-group input-group-md mb-3 d-">
                                                <ul>
                                                    <li>Project
                                                        : {{$task->subDeliverable->deliverable->project->name}}</li>
                                    ...
                                    <div class="col-md-12  table-responsive style-scroll">
                                        ...
                                        <table class="table table-striped table-bordered miw-500 dac_table"
                                                    ...
                                                    <tr>
                                                        <td>
                                                            <div class="input-group input-group-md mb-3 d-">
                                                                @if($requisition->status == 1)
                                                                    <input type="text" name="category_id"
                                                                           value="{{isset($requisitionItem->product->category->name)?$requisitionItem->product->category->name:''}}"
                                                                           class="form-control" readonly>
                                                                @else
                                                                    <select name="sub_category_id[]"
                                                                            id="subCategoryId_{{$key}}"
                                                                            class="form-control subcategory"
                                                                            onchange="getProduct($(this))"
                                                                            data-selected="{{ $requisitionItem->product->category_id }}">
                                                                        {!! $subCategories !!}
                                                                    </select>
                                                                @endif
                                                            </div>
                                                        ...
                                                        <td>
                                                            <div class="input-group input-group-md mb-3 d-">
                                                                @if($requisition->status==1)
                                                                    <input type="text" name="category_id"
                                                                           value="{{isset($requisitionItem->product->name)?$requisitionItem->product->name:''}}"
                                                                           class="form-control" readonly>
                                                                @else
                                                                    <select name="product_id[]" id="product_{{$key}}"
                                                                            class="form-control product existing-products requisition-products"
                                                                            data-selected-product="{{ $requisitionItem->product_id }}"
                                                                            data-selected-sub-category="{{ $requisitionItem->product->category_id }}"
                                                                            required onchange="getAttributes($(this))"
                                                        ...
                                                        <td>
                                                            <div class="input-group input-group-md mb-3 d-">
                                                                <input type="number" @if($modifiedName) readonly
                                                                       name="old_qty[]"
                                                                       ...
                                                                       aria-label="Large"
                                                                       aria-describedby="inputGroup-sizing-sm" required
                                                                       {{($requisition->status==1)?'readonly':''}} onchange="calculateTotal($(this))"
                                                        ...
                                                        <td>
                                                            <div class="input-group input-group-md mb-3 d-">
                                                                <input type="number" @if($modifiedName) readonly
                                                                       name="old_unit_price[]"
                                                                       value="{{ old('unit_price', $requisitionItem->unit_price) }}"
                                                                       @else name="unit_price[]"
                                                                       value="{{ old('unit_price', $requisitionItem->unit_price) }}"
                                                                       @endif  min="0" step="any"
                                                                       id="unit_price_{{$key}}"
                                                                       class="form-control requisition-unit_price text-right"
                                                                       aria-label="Large"
                                                                       aria-describedby="inputGroup-sizing-sm" required
                                                                       {{($requisition->status==1)?'readonly':''}} onchange="calculateTotal($(this))"
                                                                       onkeyup="calculateTotal($(this))">
                                                        ...
                                                        <td>
                                                            @if($requisition->status !=1)
                                                                <a href="javascript:void(0);" id="remove_{{$key}}"
                                                                   class="remove_button btn btn-danger btn-sm"
                                                                   style="margin-right:17px;" title="Remove">
                                            ...
                                            <tfoot>
                                            <tr>
                                                <td colspan="{{ auth()->user()->hasPermissionTo('department-requisition-edit') && $requisition->author_id != auth()->user()->id ? 6 : 5 }}"
                                                    class="text-right"><strong>Total Budgeted Amount:</strong></td>
                                        ...
                                        @if(auth()->user()->id == $requisition->author_id)
                                            @if($requisition->status !=1)
                                                <a href="javascript:void(0);" style="margin-right:27px;"
                                                   class="add_button btn btn-sm btn-primary pull-right"
                                                        ...
                                                        @if(isset($financeUsers[0]))
                                                            @foreach($financeUsers as $user)
                                                                <option value="{{ $user->id }}"
                                                                        {{$requisition->assigned_finance_id==$user->id?'selected':''}}>{{
                                                                $user->name }}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                    ...
                                    $errors->first('explanations').'</span>':'' !!}</p>
                                        <div class="form-group form-group-lg mb-3 d-">
                                            ...
                                            <textarea rows="3" name="explanations" id="explanations"
                                                      ...
                                                      required>{!! old('explanations', $requisition->explanations) !!}</textarea>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6 mt-3">
                                        ...
                                        <div class="form-group form-group-lg mb-3 d-">
                                            <textarea rows="3" name="remarks" id="remarks"
                                                      class="form-control rounded word-restrictions" aria-label="Large"
                                                      aria-describedby="inputGroup-sizing-sm"
                                                      data-input="recommended">{!! old('remarks',$requisition->remarks) !!}</textarea>
                                        </div>
                                        ...
                                        @if($modifiedName)
                                            ...
                                            <input type="hidden" name="deliverable_id"
                                                   value="{{$requisition->deliverable_id}}">
                                        @else
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-6">
                                        <button type="button" onclick="updateRequisition()"
                                                class="btn btn-primary rounded pull-right update-button"><i
                                    ...
                                    <div class="col-md-6 {{ in_array($log->type, ['department-head']) ? 'offset-md-6' : '' }}">
                                        <div class="panel">
                                            <div class="panel-body">
                                                <p>{{ $log->notes }}</p>
                                                <br>
                                                <small>{{$log->createdBy->name}}
                                                    &nbsp;&nbsp;|&nbsp;&nbsp;{{ ucwords(implode(' ', explode('-', $log->type))) }}
                                                    &nbsp;&nbsp;|&nbsp;&nbsp;{{ date('Y-m-d g:i a', strtotime($log->created_at))}}</small>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
