            ...
            <div class="invoice-details mt25 row">
                <div class="well col-12">
                    <ul class="list-unstyled mb0">
                        <li><strong>{{__('Product Name') }} :</strong> {{$product->name}} ({{$product->sku}}) {{ getProductAttributesFaster($product) }}</li>
                        <li><strong>{{__('Total Requisition') }} :</strong> {{$product->requisitionItem()->where('is_send','no')->whereIn('requisition_id',$requisitionIds)->count()}}</li>
                    </ul>
                </div>
            </div>
        </div>
            ...
            <table class="table table-striped table-bordered miw-500 dac_table" cellspacing="0" width="100%" id="dataTable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{isset($item->requisition->reference_no)?$item->requisition->reference_no:''}}</td>
                        <td>{{isset($item->requisition->relUsersList->name)?$item->requisition->relUsersList->name:''}}</td>
                        <td>
                        ...
                        <td class="text-center">{{($sumOfSendRFP >0)?$item->qty-$sumOfSendRFP:$item->qty}}</td>
                    </tr>
                    @endforeach
                    <tr>
                        <td colspan="5" class="text-right"><strong>Total</strong></td>
                        <td class="text-center">{{$items->sum('requisition_qty')}}</td>
                        <td class="text-center">{{$items->sum('qty')}}</td>
                        <td class="text-center">{{($sumOfSendRFP >0)?$totalSumOfSendRFP:$items->sum('qty')}}</td>
                    </tr>
