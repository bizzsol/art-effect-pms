...
<div class="main-content">
    <div class="main-content-inner">
        ...
        <div class="page-content">
                    ...
                    <table class="table table-striped table-bordered table-head " cellspacing="0" width="100%" id="dataTable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><a href="javascript:void(0)" data-src="{{route('pms.requisition.list.view.show',$values->relRequisition->id)}}" class="btn btn-link requisition m-1 rounded showRequistionDetails">{{ isset($values->relRequisition->reference_no)?$values->relRequisition->reference_no:'' }}</a>
                                </td>
                                <td class="text-center">{{ isset($values->relRequisition->relUsersList->name)?$values->relRequisition->relUsersList->name:'' }}</td>
                                <td class="text-center">{{isset($values->relRequisition->items)? $values->relRequisition->items->sum('qty'):0}}</td>
                                <td class="text-center" id="status{{isset($values->relRequisition->id)?$values->relRequisition->id:0}}">
            ...
            <div class="panel panel-info">
                <div class="panel-body">
                    <h4 class="pb-1">Quotation List Based On Proposal</h4>
                    <table class="table table-striped table-bordered table-head " cellspacing="0" width="100%" id="dataTable">
                        <thead>
                            <tr class="text-center">
                                <th width="5%">{{__('SL')}}</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if(isset($purchaseOrder->relQuotation->id))
                                    @if($values->id==$purchaseOrder->relQuotation->id)
                                    <a href="javascript:void(0)" data-title="Quotation CS" class="btn btn-success btn-xs">Approved</a>
                                    @endif
                                    @endif
                                    @if(!auth()->user()->hasRole('Employee'))
                                    <a target="__blank" href="{{url('pms/quotation/cs-compare-view-pdf/'.$values->request_proposal_id)}}" class="btn btn-warning btn-xs" title="History"><i class="las la-user-clock"></i></a>
                                    @endif
                                </td>
            ...
            @if(!empty($purchaseOrder) && count($purchaseOrder->relGoodReceiveNote)>0)
            <div class="panel panel-info">
                <div class="panel-body">
        ...
        <div class="panel panel-info">
            @php
            $qualityEnsure = collect($purchaseOrder->relGoodReceiveNote);
            @endphp
            <div class="panel-body">
