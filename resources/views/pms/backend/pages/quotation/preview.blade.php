    ...
    <table class="table preview-table table-bordered table-hover" style="width: {{ ($quotations->count()*300)+500 }}px">
        ...
        @if(isset($quotation->id))
            @foreach($quotation->relQuotationItems as $key=>$item)
                <tr>
                    ...
                    @foreach($quotations as $key=>$quotation)
                        <td class="text-right">
                            {{ systemMoneyFormat($quotationInfo[$quotation->id]['items'][$item->uid]['unit_price']) }}
                        </td>
                        <td class="text-right">
                            {{ systemMoneyFormat($quotationInfo[$quotation->id]['items'][$item->uid]['sub_total']) }}
                        </td>
                        @if($systemCurrency->code != ($quotation->exchangeRate ? $quotation->exchangeRate->currency->code : ''))
                            <td class="text-right">
                                {{ systemMoneyFormat($quotationInfo[$quotation->id]['items'][$item->uid]['exchange_sub_total']) }}
                            </td>
                        @endif
                    @endforeach
                </tr>
        ...
        <tr>
            ...
            @foreach($quotations as $key=>$quotation)
                ...
                <td class="text-right">
                    <strong>{{ systemMoneyFormat($quotationInfo[$quotation->id]['sub_total']) }}</strong>
                </td>
                @if($systemCurrency->code != ($quotation->exchangeRate ?
                $quotation->exchangeRate->currency->code : ''))
                    <td class="text-right">
                        <strong>{{ systemMoneyFormat($quotationInfo[$quotation->id]['exchange_sub_total']) }}</strong>
                    </td>
                @endif
            @endforeach
        </tr>
