                ...
                <div class="invoice-details mt25 row">
                    <div class="well col-6">
                        <ul class="list-unstyled mb0">
                            ...
                            @if(isset($quotations->relSupplierPaymentTerm->id))
                                <li><strong>Payment Term
                                        :</strong> {{makePaymentTermsString($quotations->relSupplierPaymentTerm->id)}}
                                </li>
                            @endif
                        </ul>
                    </div>
                    <div class="col-6">
                        <ul class="list-unstyled mb0 pull-right">
                            <li><strong>{{__('Date')}}
                                    :</strong> {{date('d-m-Y',strtotime($quotations->quotation_date))}}</li>
                            @if(!empty($quotations->delivery_date))
                                <li><strong>{{__('Delivery Date')}}
                                        :</strong> {{date('d-m-Y',strtotime($quotations->delivery_date))}}</li>
                            @endif
                            ...
                            <li><strong>{{__('Generated By')}}
                                    :</strong> {{$quotations->relRequestProposal->createdBy->name}}</li>
                            <li>Validated for <strong>{{ inWordBn($quotations->validation_days, true, '') }}</strong>
                                days
                            </li>

                            @if($quotations->quotation_file)
                                <li><a href="{{asset($quotations->quotation_file)}}"
                                       target="__blank"><strong>{{__('Quotation File')}}</strong></a></li>
                            @endif

                            @if($quotations->technical_specification_file)
                                <li><a href="{{asset($quotations->technical_specification_file)}}"
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover">
                    ...
                    @foreach($quotationItems as $key=>$item)
                        <tr>
                            ...
                            <td>{{isset($item->relProduct->name)?$item->relProduct->name:''}}
                                ...
                                ) {{ getProductAttributesFaster($requisitionItems->where('product_id', $item->product_id)->where('uid', $item->uid)->first()) }}</td>
                    ...
                    <tr>
                        ...
                        <td class="text-right">{{number_format($quotations->relQuotationItems->sum('unit_price'),2)}}</td>
