                                    ...
                                    @foreach($proposals->requestProposalRequisition as $key => $values)
                                        <tr>
                                            ...
                                            <td><a href="javascript:void(0)"
                                                   data-src="{{route('pms.requisition.list.view.show',$values->relRequisition->id)}}"
                                                   class="btn btn-link requisition btn-xs m-1 rounded showRequistionDetails">{{ isset($values->relRequisition->reference_no)?$values->relRequisition->reference_no:'' }}</a>
                                            </td>
                                    ...
                                    @if($proposals->relQuotations->count() > 0)
                                        @foreach($proposals->relQuotations as $rkey => $values)
                                            ...
                                            <tr>
                                                ...
                                                @if($rkey == 0)
                                                    <td rowspan="{{ $proposals->relQuotations->count() }}"><a
                                                                href="javascript:void(0)"
                                                                onclick="openFullViewModal({{$values->request_proposal_id}})"
                                                                class="btn btn-link">{{$values->relRequestProposal->reference_no}}</a>
                                                    </td>
                                                ...
                                                <td>
                                                    <a href="javascript:void(0)" onclick="openModal({{$values->id}})"
                                                       class="btn btn-link">{{$values->reference_no}}</a>

                                                    @if(isset($purchaseOrder->relQuotation->id) && $values->id==$purchaseOrder->relQuotation->id)
                                                        <a href="javascript:void(0)" data-title="Quotation CS"
                                                           class="btn btn-success btn-xs">Approved</a>
                                                    @endif

                                                    <a target="__blank"
                                                       href="{{url('pms/quotation/cs-compare-view-pdf/'.$values->request_proposal_id)}}"
                                                       class="btn btn-warning btn-xs" title="Quotation CS History"><i
                            ...
                            <div class="panel-body">
                                ...
                                <table class="table table-striped table-bordered table-head " cellspacing="0"
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="javascript:void(0)" class=" btn-link showPODetails"
                                               data-src="{{route('pms.purchase.order-list.show',$purchaseOrder->id)}}">{{$purchaseOrder->reference_no}}</a>
                                        </td>
                                ...
                                <table class="table table-striped table-bordered table-head " cellspacing="0"
                                    ...
                                    @foreach($purchaseOrder->relGoodReceiveNote as $key=> $grn)
                                        <tr>
                                            ...
                                            @if($key == 0)
                                                <td rowspan="{{ $purchaseOrder->relGoodReceiveNote->count() }}"><a
                                                            href="javascript:void(0)" class="btn btn-link"
                                                            onclick="purchaseOrderDetails($(this))"
                                                            data-src="{{route('pms.purchase.order-list.show', $grn->relPurchaseOrder->id)}}"
                                                            data-title="Purchase Order Details">{{$grn->relPurchaseOrder->reference_no}}
                                                    </a>
                                                </td>
                                            @endif
                                            <td>
                                                ...
                                                <div style="width: 100%">
                                                    <div style="width: 90%;float: left;clear:right">
                                                        <a href="javascript:void(0)" class="btn btn-link"
                                                           onclick="showGateInPODetails($(this))"
                                                           data-src="{{route('pms.grn.grn-process.show',$grn->id)}}"
                                                           data-title="Gate In Details">{{$grn->reference_no}}</a>
                                                    </div>
                                                    <div style="width: 10%;float: left;clear:right">
                                                        <a class="btn btn-primary btn-xs"
                                                           href="{{ url('pms/grn/gate-in-slip/'.$grn->purchase_order_id.'?grn='.$grn->id) }}"
                                                           target="_blank" style="margin-top: 7.5px"><i
                                    ...
                                    @if($grn->relGoodsReceivedItems->where('quality_ensure','approved')->sum('qty')>0)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @if($count > 0)
                                                    <a href="{{route('pms.quality.ensure.approved.single.list',$grn->id)}}"
                                                       class="btn btn-xs btn-info">Items ({{$count}})</a>
                                                @endif
                                                <a target="_blank"
                                                   href="{{route('pms.quality.approved.item.print',['id'=>$grn->id,'type'=>'approved'])}}"
                                                   title="Approved List" class="btn btn-xs btn-success"><i
                                    ...
                                    @if($grn->relGoodsReceivedItems->where('quality_ensure','return')->sum('qty')>0)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @if($returned > 0)
                                                    <a href="'.route('pms.quality.ensure.return.single.list',$grn->id).'"
                                                       class="btn btn-xs btn-info">Items ({{$returnedCount}})</a>
                                                    <a target="__blank"
                                                       href="{{route('pms.quality.return.item.print',['id'=>$grn->id,'type'=>'return'])}}"
                                                       class="btn btn-xs btn-warning" title="Return List"><i
                                                                class="las la-print"></i></a>
                                                @endif

                                                @if($approved > 0)
                                                    <a target="__blank"
                                                       href="{{route('pms.quality.return.item.print',['id'=>$grn->id,'type'=>'return-approved-list'])}}"
                                                       class="btn btn-xs btn-success" title="Return Approved List"><i
                                                                class="las la-print"></i></a>
                                                @endif
                                            </td>
                                        </tr>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @if(auth()->user()->hasPermissionTo('quality-ensure-return-change-received-list') && $count > 0)
                                                    <a href="{{route('pms.quality.ensure.return.change.single.list',$grn->id)}}"
                                                       class="btn btn-xs btn-info">{{__('Items')}} ({{$count}})</a>
                                                @endif

                                                @if($returned > 0)
                                                    <a target="__blank"
                                                       href="{{route('pms.quality.return.replace.item.print',['id'=>$grn->id,'type'=>'return-change-list'])}}"
                                                       title="Return Replace List" class="btn btn-xs btn-warning"><i
                                                                class="las la-print"></i></a>
                                                @endif

                                                @if($approved > 0)
                                                    <a target="__blank"
                                                       href="{{route('pms.quality.return.replace.item.print',['id'=>$grn->id,'type'=>'return-change'])}}"
                                                       title="Return Replace Approved List"
                                                       class="btn btn-xs btn-success"><i class="las la-print"></i></a>
                                                @endif
                            ...
                            <div class="panel-body">
                                ...
                                <table class="table table-striped table-bordered table-head " cellspacing="0"
                                    ...
                                    <tr>
                                        <td class="text-center">1</td>
                                        <td><a href="javascript:void(0)" class="btn-link showPODetails"
                                               onclick="showPODetails($(this))"
                                               data-src="{{route('pms.purchase.order-list.show',$billManage->id)}}">{{$billManage->reference_no}}</a>
                                        </td>
                                        ...
                                        <td>
                                            @if(isset($billManage->relPoAttachment[0]))
                                                ...
                                                @if(!empty($poAttachment->invoice_file))
                                                    <a href="{{asset($poAttachment->invoice_file)}}" target="__blank"
                                        ...
                                        <td>
                                            @if(isset($billManage->relPoAttachment[0]))
                                                ...
                                                @if(!empty($poAttachment->vat_challan_file))
                                                    <a href="{{asset($poAttachment->vat_challan_file)}}"
                                                       target="__blank" class="btn btn-success btn-xs"
                                                       title="Click here to view vat chalan"><i
                                                                class="las la-money-check-alt"></i></a>
